...
def main() -> None:
    ...
    ap.add_argument("--images", required=True, help="학습 이미지 zip 경로")
    ap.add_argument("--destination", required=True, help="결과 모델 owner/name (미리 만들어 둘 것)")
    ap.add_argument("--trigger", default=None, help="트리거 단어 (예: PAWDYWINTER)")
    ap.add_argument("--steps", type=int, default=1000)
    ap.add_argument(
        "--trainer",
        default=os.getenv("PETFIT_REPLICATE_TRAINER", "replicate/fast-flux-trainer:latest"),
        help="트레이너 모델:버전",
    )
    ap.add_argument("--input-key", default="input_images", help="이미지 zip 을 넣을 input 키")
    ...
    if args.trigger:
        print(f'    PETFIT_LOOK_TRIGGERS=\'{{"{args.look}": "{args.trigger}"}}\'')
