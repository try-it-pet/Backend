...
> 최종 업데이트: **2026-07-08** (아래 최신 섹션부터 읽을 것. 그 아래 히스토리)

---

# 🟣 2026-07-08 — 펫 LoRA 프리미엄 검증 + 화풍(그림풍) 방향 (이 세션 최신)

## 1) per-pet LoRA(펫 전용 모델) = 프리미엄 → 품질 검증 **성공**
- 실제 펫 **몽자**(브라운 토이푸들) 사진으로 LoRA 학습해 프리미엄 가치 검증 완료. 상세·결정은 메모리 [[per-pet-lora-premium]].
- **학습**: **fal.ai** `fal-ai/flux-lora-fast-training`(트리거 `MONGZADOG`, 정제 7장, 몇 분). ⚠️Replicate `fast-flux-trainer`는 큐 55분 밀려서 **취소하고 fal로 전환** → **학습은 fal 확정**(추론은 그대로 Replicate).
- **데이터 정제**: 유튜브/게시글 캡처 10장 → 자막·UI·여러마리 크롭/드롭 → **깨끗한 7장**(캡션 있으면 LoRA가 그것까지 외움). 스크립트: scratchpad `mongza_clean/`.
- **검증 결과(대성공)**: ① txt2img "MONGZADOG" → 안 찍은 장면(방/스튜디오/겨울/벚꽃)도 몽자로 일관. ② **풀 파이프라인**(펫LoRA로 깨끗한 몽자 생성→실제 상품 착용→감성룩): 후디+겨울·SKI니트+벚꽃 2조합 모두 **옷·정체성·자연스러움 3개 동시 성공**.
- **핵심 발견**: LoRA로 **깨끗한 base 생성→옷 입히기**가 사용자 사진 편집보다 **옷 정확도까지 높임**(지울 기존옷·배경잡음 없음). → 펫 LoRA가 3가지 다 끌어올림.
- **제품화(품질 확인됐으니 조립)**: 펫 등록에 사진 여러 장 → **비동기 학습('굽는 중'+완료 푸시)** → LoRA를 펫 에셋으로 R2 저장(fal.media 임시URL 재호스팅) → 생성 시 자동 적용 → 결제. **온보딩 블로킹 금지**(기본은 즉시 편집, LoRA는 옵트인 프리미엄).

## 2) 친구 논의 후 확정된 제품 방향 7
1. 펫 LoRA=**프리미엄**(품질 검증됨). 2. 룩 가치=표정/구도가 아니라 **사진풍·그림체 자체가 바뀌게**. 3. 그림체는 **더 과장 귀엽고 미형**(=화제성). 4. **화풍 학습 레퍼런스 큐레이션이 관건**. 5. 인생네컷=4장 업로드 X → **펫 LoRA로 고정 포즈/정해진 구도 4컷**. 6. 단 인생네컷에 **전신컷 ≥1(제품 착용) 필수**. 7. **모든 출력 일그러짐 없이**.
> ⚠️ #5로 인해 **방금 만든 '4장 업로드 인생네컷'(커밋 b333cf7)은 방향 폐기** → pet-LoRA 고정템플릿으로 재설계 예정.

## 3) 화풍/그림풍 탐색 (심슨 → 우리식)
- **심슨 그대로는 X**: 프롬프트론 정확한 심슨 안 나오고 일반 카툰化 + **심슨=Disney IP 저작권 문제**. → **레퍼런스로만 쓰고 우리 화풍으로 변형**.
- **핵심 발견**: **사진 펫 LoRA + 강한 그림체는 충돌**(강도↑=사진, 강도↓=스타일·정체성↓). 그림체는 **redraw(Kontext 편집)**로 따로 가야 함. 즉 **펫 LoRA=사진풍 정체성 / redraw·화풍 LoRA=그림체** = 서로 다른 도구, 섞지 말 것.
- **탐색 결과(scratchpad)**: 그림체 4 `style_{kawaii,flat,plush,bold}.png` + 그림풍 4 `genre_{watercolor,retroanime,claymation,pixelart}.png`. 다 무왜곡.
  - **프롬프트로 바로 되는 것**(강·그래픽 매체): 카와이 스티커·볼드 카툰·**클레이**·**픽셀**.
  - **화풍 LoRA 필요**(은은한 회화체): **수채화 동화·특정 애니체** = 프롬프트론 일반 카툰으로 뭉개짐 → #4 큐레이션 후 학습해야 진짜로 나옴.
- **다음**: 라인업(그림체+그림풍 섞기) 확정 → 즉시가능은 룩으로 추가, 시그니처 1~2개는 레퍼런스 큐레이션→화풍 LoRA. **(사용자 픽 대기 중)**

## 4) 이번 세션 커밋(푸시됨, main)
전처리·인생네컷안정화·프롬프트·업스케일 → LoRA 추론튜닝 → SCENE버그 → 2단계피팅 → 벚꽃룩 → 2단계quota → 인생네컷품질 → 벚꽃'원본그대로'수정 → OpenAI제거(moondream) → 프론트 replicate기본 → 사진변경(#2) → 4장인생네컷(#1, ⚠️방향폐기) → fourcut실물옷. **프로덕션 검증 완료**(replicate·2단계·R2). 버전 v1.2.0 APK 빌드됨.
> ⚠️ 미커밋: pubspec/settings 버전 1.2.0 표기 변경은 커밋 안 됨. 화풍/펫LoRA 실험물은 전부 scratchpad(레포 아님).
